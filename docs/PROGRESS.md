...
- Full experiment: `scripts/run_batch.py` creates per-density/seed run directories and keeps SUMO and OMNeT++ seeds matched across the four approaches. `scripts/run_full_batch.sh` is running in the active WSL session. At this checkpoint, four of 360 batch scalar files are complete (all four configurations for low density, seed 1), and the low-density seed-2 work has begun. Its session ID in the current tool environment is `69036`. Raw batch outputs use `results/raw/<configuration>-<density>-seed<seed>.{sca,vec,vci}`; batch logs use `artifacts/logs/batch/`. The runner skips complete runs on restart. Run `python analysis/process_results.py --batch --require-all` only after all 360 raw scalar files exist.
